...
void ReadTypesFile(void) {       // read the type_file: including atom_name,res_name and type_num; Also refresh MAX_TYPES
  int type_num, i;
  char res_name[4], atom_name[4];  // twenty_res_typing
  FILE *type_file;
  ...
  while(fscanf(type_file,"%s %s %d", atom_name, res_name, &type_num)!= EOF) {
    ++i;
    strcpy(atom_type_list[i].atom_name, atom_name);   // copy atom_name into atom_type_list (struct)
    strcpy(atom_type_list[i].res_name, res_name);
    atom_type_list[i].type_num = type_num;
    if (!strcmp(res_name,"XXX")) {
      if (!strcmp(atom_name,"N"))
	bb_N_type = type_num;     // bb_N_type? O_type? OXT_type?
  ...
  natom_type_list = i+1;    //  Because it begins from 0.   In addition, total items are more than 84 kinds! see the file, some items has the same type_num
  MAX_TYPES = atom_type_list[i].type_num + 1;  // total 84 kinds
  fclose(type_file);
}

void SetupMuPotential (void) { // calcluate mu potential: the first item of total energy, the contact energy, defination of mu
  int i, j, k, l;
  Float n_nat, avg;
  int **nat_cons, **nnat_cons;  // both: max_types*max_types matrix  record contacts or not
  ...
  for (k = 0; k < natoms; ++k) 
    for (l = k+1; l < natoms; ++l) {
      if (data[k][l].contacts) {               //  where is data (contact matrix) from
	if (native[k].smogtype <= native[l].smogtype) // smogtype: maybe arangement indices of 20 aa
  ...
  mu = (avg/(float) n_nat)/(1 + avg/(float) n_nat); // defination of mu 
  fprintf(STATUS,"mu: %f\n", mu);

  for (i = 0; i < MAX_TYPES; ++i)
    for(j = i; j < MAX_TYPES; ++j)
      if ((nat_cons[i][j] !=0) || (nnat_cons[i][j]!=0)) {
	if (mu == 1) {    // why mu could equal 1?
	  if (nat_cons[i][j] != 0)
	    potential[i][j] = -1;
	  else
	    potential[i][j] = 1;
	  ...
	  potential[i][j] = ((1-mu)*nnat_cons[i][j] - mu*nat_cons[i][j])/(mu*nat_cons[i][j] + (1-mu)*nnat_cons[i][j]); // mu potential
...
void InitializeProtein() {   // Very Important!! Center!
  int i;
  /* Reset variables */

  fprintf(STATUS,"---MODEL---\n");
  fprintf(STATUS,"  file:\t\t%s   \n", native_file);
  native = (struct atom *) calloc(MAX_ATOMS,sizeof(struct atom)); // array of struct(atom)               
  ...
  amino_acids = (struct amino *) calloc(20, sizeof(struct amino));  // 20 kinds of aa
  fprintf(STATUS,"Made amino_acids structure\n");

  ReadTypesFile();
  fprintf(STATUS,"Read Types File\n");
  ReadHelicityData();  // align.h
  ...
  nresidues=0;
  for(i=0; i<natoms; i++) if (!strncmp(native[i].atomname,"CA",2)) nresidues++;   // strncmp:compare first 2 letters between them
  if (nresidues!=(native[natoms-1].res_num+1)) fprintf(STATUS,"FILE: %s   MISSING RESIDUES!!!\n", native_file);
  ...
  CenterProtein(&native,natoms);  // to locate the protein in the center (0,0,0)
  for(i=0;i<natoms; i++) 
  ...
  CheckCorrelation(data,native,native_residue,natoms);   // whether check contacts, clashes or ohter 
  ...
  initialize_torsion();      // these four potentials are all in loop.h
  initialize_sct();
  initialize_aromatic();
  initialize_secstr();
  read_cluster();    //  loop.h
  if (weight_hbond){
    InitializeHydrogenBonding();  // hbonds.h
  ...
  Contacts();  // initialize contacts setting in contacts.h

  /* Setup potential and related structures */

  SetupAlignmentStructure();  // align.h

  if (USE_GO_POTENTIAL){
    mu = 1;
    SetupAlignmentPotential();  // align.h
  }
...
//  }
  ...
  TypeContacts();  // record the type of atoms for each contact ; contacts.h
  native_E = FullAtomEnergy();   // erergy (for different pair of atom types) * pair of atom types
...
void SetHardCore() {
  ...
  hard_core = (long int **) calloc(MAX_TYPES,sizeof(long int *));   // MAX_TYPES*MAX_TYPES matrix
  for(i=0; i<MAX_TYPES; i++)
  ...
  for(i=0; i<MAX_TYPES; i++)
	...
	rad2 = radii[native[j].atomtype];
      ...
      hard_core[i][j] = (long int) (temp*temp*INT_PRECISION*INT_PRECISION);   // INT_PRECISION in define.h
    }
...
void SetContactDistance() {
  ...
  for(i=0; i<MAX_TYPES; i++)
	...
	rad2 = radii[native[j].atomtype];
      ...
      contact_distance[i][j].b = (long int) (temp*temp*INT_PRECISION*INT_PRECISION);   // contact reterion: [a,b]
...
void SetRadii() {        // maybe the atomtype is total 14 kinds
...
void ReadNative(char *file_name, struct atom *protein, int *Natoms) {  // read the pdb file
  FILE *the_file;
  char line[250];
  ...
  *Natoms = 0;
  ...
  while(fgets(line,100,the_file) != NULL) // read 99 chars each line
...
void GetResidueInfo(struct atom *Chain, struct residue *Residue, int Nres, int Natoms) {
  ...
  for(i=0; i<Natoms; i++) {
    j = MatchAtomname(Chain[i].atomname);  // j stands for a kind of atom, range from 0 to 39?
...
void GetChi() { /* Chi angles are stored in radians */
  ...
  for(i=0; i<nresidues; i++) 
    for(j=0; j<amino_acids[native_residue[i].amino_num].ntorsions; j++) {
      ...
      native_residue[i].tmpchi[j] = native_residue[i].chi[j];   // why use three arrays?
    }
  
  return;
}

void ReadAvgChis (void) {     // no_chi_list
  int i; 
  ...
  if((DATA = fopen(rotamer_data_file,"r"))==NULL)   // bbind02.May.lib
   {
    fprintf(STATUS,"ERROR: Can't open the file: %s!\n", rotamer_data_file);
    exit(1);
   }
...
void ReadSidechainTorsionData() {           // amino_torsion.data    // ntorsions: how many torsion angles in the side chain of an amino acid 
  int temp_x, i, j, k;                      // ntotamers equal ntorsions for most of aa, but some are exceptional, see below line593-601
  short temp;
  ...
  while (fgets(line,150,DATA)!=NULL) {
	...
	sscanf(line,"%d %s %*d %d %*s %s",&i, name, &j, symbol);    // %*d :skip this num in the input
...
void ReadPotential(){           // p178_conrange4_potential_0.995054  record the potential energy of a pair of 84 kinds atoms (84*84)
  FILE *pot_file;
  int i, j;
  float val;
...
void InitializeData() {
  ...
  debug_dcontacts = (unsigned char **) calloc(natoms,sizeof(unsigned char *));   // dcontacts??
  for(i=0; i<natoms; i++)
...
void DetermineTriplets() {   // record coordinates of a single residue/ double residues or a triplet
  int i, j, k;

  residue_triplets = (struct triplet *) calloc((nresidues-5)*81, sizeof(struct triplet));   //
  /* this is way more memory than needed, given that the exact number of triplets is 16n - 55 */   // yes! single:n; double:5n-15; triple:10n-40
  total_triplets = 0;
  for(i = 0; i <nresidues; i++) {                    // for single
  ...
  for(i = 0; i < nresidues; i++)          // for double and local (residue seperate<6)
  ...
  TOTAL_DOUBLE_LOOP_MOVES = total_triplets-TOTAL_SINGLE_LOOP_MOVES;  // euqals num of local pair residues   
  
  for(i = 0; i < nresidues; i++)  // for triple and local (three residues are all <6 residue seperate)
    for(j = i+1; j < nresidues; j++)
...
void InitializeBackboneRotationData() {
  int i, j, k;
  // !!! line 731-794 To define a global move that rotate the short part of the chain diveded by the residue; line796-944: a loop move for each residue in the single, double or triple based on the previous defination; line 945-952: init_rotate: to rotate the psi angle for the first residue or the phi angle for the last residue of the chain: that's why for the latter part of chain ,fist record phi then psi ;and for the former part of the chain, first psi then phi !!! 
  ...
  rotate_natoms = (short **) calloc(2, sizeof(short *));     //2*n1    where n1:nresidues n2:natoms 
  rotate_atom = (short ***) calloc(2, sizeof(short **));     // 2*n1*n2
  not_rotated = (char ***) calloc(2,sizeof(char **));        // 2*n1*n2
  for(i=0; i<2; i++) {
  ...
  /* rotate the short end of the chain for each residue */   // Yes!! Only rotate the short part of the chain diveded by the residue
  ...
  for(i=0; i<nresidues; i++) {
    ...
    if (i > nresidues/2.0) { 
      for(j=0; j<natoms; j++)
	if (native[j].res_num > i) {  // record need to rotate psi and phi (mark with the larger atom index) for each pair of atoms of the latter part residues of the chain
	  rotate_atom[0][i][rotate_natoms[0][i]++] = j;  // rotate_natoms[0][i]++ : the index for residue i : begin with 0
	  rotate_atom[1][i][rotate_natoms[1][i]++] = j;  // same to above
	}	
	else if (native[j].res_num == i) { // record need to rotate psi and phi (mark with the larger atom index) for each pair of atoms fot each pair of atoms in the same residue except for N and CA
    ...
    else {            // same to above; the two parts tell us only rotate the short part of the chain devided by a residue
      for(j=0; j<natoms; j++)   
  ...
  for(i=0; i<nresidues; i++) {
    ...
    for(j=0; j<natoms; j++) {
      ...
      not_rotated[1][i][j]=!not_rotated[1][i][j];            //  yes! 0:rotated 1:not rotate
    }
  }

  /* store atoms rotated by loop moves */
  // should to know the differences of these 5 matrices, group1: for number, 1,2   total_triplets*6  ; group2:3,4,5    total_triplets*6*natoms
  ...
  for(i=0; i<total_triplets; i++){
    /* there are up to 6 bonds to rotate for each loop move */  // single:phi+psi; double:2*(phi+psi); triple:3*(phi+psi)
  ...
  for(i=0; i<total_triplets; i++) {
	...
	loop_rotate_atoms[i][0][j] = rotate_atom[1][residue_triplets[i].a][j];  // record all need to rotate phi of atoms (mark with the larger atom index) for a single  
      loop_rotate_natoms[i][1] = rotate_natoms[0][residue_triplets[i].a]; /* psi */
      for(j=0; j < loop_rotate_natoms[i][1]; j++) 
	loop_rotate_atoms[i][1][j] = rotate_atom[0][residue_triplets[i].a][j];  // record all need to rotate psi of atoms (mark with the larger atom index) for a single  

      if (residue_triplets[i].b >=0) {
	loop_rotate_natoms[i][2] = rotate_natoms[1][residue_triplets[i].b]; /* phi */ // record all need to rotate phi of atoms (mark with the larger atom index) for the second residue of a double 
	for(j=0; j < loop_rotate_natoms[i][2]; j++) 
	  loop_rotate_atoms[i][2][j] = rotate_atom[1][residue_triplets[i].b][j];
	loop_rotate_natoms[i][3] = rotate_natoms[0][residue_triplets[i].b]; /* psi */ // record all need to rotate psi of atoms (mark with the larger atom index) for the second residue of a double 
	...
	loop_rotate_natoms[i][3] = 0;
      ...
      if (residue_triplets[i].c >=0) {
	loop_rotate_natoms[i][4] = rotate_natoms[1][residue_triplets[i].c]; /* phi */  // record all need to rotate phi of atoms (mark with the larger atom index) for the third residue of a triple
	for(j=0; j < loop_rotate_natoms[i][4]; j++) 
	  loop_rotate_atoms[i][4][j] = rotate_atom[1][residue_triplets[i].c][j];
	loop_rotate_natoms[i][5] = rotate_natoms[0][residue_triplets[i].c]; /* psi */  // record all need to rotate psi of atoms (mark with the larger atom index) for the third residue of a triple
	...
	for(k=0; k<natoms; k++)
	  if (native[k].res_num < residue_triplets[i].c) {  
	    loop_rotate_atoms[i][j][loop_rotate_natoms[i][j]++] = k;   /* psi */      // also record need to rotate psi like above for all, but more clean ,but not very clear 
	    loop_rotate_atoms[i][j+1][loop_rotate_natoms[i][j+1]++] = k;  /* phi */
	  }
	  else if (native[k].res_num == residue_triplets[i].c) {
	    if (k != native_residue[residue_triplets[i].c].O && k != native_residue[residue_triplets[i].c].C && k != native_residue[residue_triplets[i].c].CA)  // except for O, C and CA in the same residue
  ...
  for(i=0; i<total_triplets; i++)    // line945-952: init rotate :rotate the residue with only rotate phi or psi (this residue maybe in the boarder of the chain)
    for(j=5; j>0; j--) {
...
void InitializeSidechainRotationData() {
  ...
  sidechain_torsion = (short ***) calloc(nresidues,sizeof(short **));  // nresidue* (torsion kinds for a aa) * 4 (maybe a torsion angle includes 4 atoms) 
  for(i=0; i<nresidues; i++) {
    sidechain_torsion[i] = (short **) calloc(amino_acids[native_residue[i].amino_num].ntorsions,sizeof(short *));
    for(j=0; j<amino_acids[native_residue[i].amino_num].ntorsions; j++)
      sidechain_torsion[i][j] = (short *) calloc(4,sizeof(short));  // use 4 atoms to define a torsion ,see line1040-1041
  }

  sct_E = (short *****) calloc(nresidues,sizeof(short ****));  // nresidues * 12*12*12*12    ???:line 968-981
  for(i=0; i<nresidues; i++)
   {
  ...
  for(i=0; i<nresidues; i++) {
	...
	do {
	  ...
	  l -= three[j-1]*native_residue[i].rot_position[k][j-1];  // ??? equal 0
  ...
  for(i=0; i<nresidues; i++) {
    ...
    for(j=0; j<native_residue[i].ntorsions; j++) {
      rotate_sidechain_atom[i][j] = (short *) calloc(amino_acids[native_residue[i].amino_num].rotate_natoms[j],sizeof(short *));  // nresidue*ntorsions*num of atoms affected by torsion
      sidechain_not_rotated[i][j] = (char *) calloc(natoms,sizeof(char));  // nresidues*ntorsions*natoms (more memory than rotate_sidechain_atom, not rotate aotm=1;roatate atom=0,see line1070)
    }
  }
  ...
  for(i=0; i<nresidues; i++) {
	...
	for(l=0; l<natoms; l++) 
	  if (native[l].res_num == i && !strcmp(native[l].atomname,amino_acids[native_residue[i].amino_num].torsion[j][k])) {
	    ...
	    sidechain_torsion[i][j][k] = l;      // see the above note; use four atoms to define a torsion
	    break;
	  }
	if (l==natoms)
         {
	...
	for(l=0; l<natoms; l++) 
	  if (native[l].res_num == i && !strcmp(native[l].atomname,amino_acids[native_residue[i].amino_num].rotate_atom[j][k])) {
	    rotate_sidechain_atom[i][j][k] = l;  // record the atom affected by the torsion, see line 1025
	    break;
	  } 
	if (l==natoms)
         {
...
void SetProgramOptions(int argc, char *argv[]) {
  ...
  ierr=MPI_Bcast(&MPI_STOP,1,MPI_INT,0,mpi_world_comm);  // broadcast a message from the process with rank "root" to all otherprocesses of the communicator
  ...
  for(l=0;l<nprocs;l++){
    Tnode[l]=MC_TEMP_MIN * expf(l*logf(10.0)/(nprocs-1));
  ...
  while(fgets(line,150,DATA) != NULL) {
	...
	strcpy(alignment_file,name);
      ...
      else if (!strcmp(token,"FRAGLIB_FILE")) {
  sscanf(line,"%*s %s",name);
  strcpy(fraglib_file,name);
  //printf("%s\n",fraglib_file);
      }
	...
	find_yang_move = 1;
       ...
       else if (!strcmp(token,"FRAG_MOVE"))
       {
  FRAG_MOVE = value;
  find_frag_move = 1;
       }
       else if (!strcmp(token,"USE_CRANK_MOVE"))
       {
  USE_CRANK_MOVE = value;
       }
       else if (!strcmp(token,"NOISE_RANGE_PHI"))
       {
  NOISE_RANGE_PHI = value;
       }
       else if (!strcmp(token,"NOISE_RANGE_PSI"))
       {
  NOISE_RANGE_PSI = value;
       }
  ...
  if (find_yang_scale == 0)
   ...
   if (find_frag_move == 0)
   {
    fprintf(STATUS,"There is nothing on FRAG_MOVE!\n");
    exit(1);
   }
  /* lattice parameters */
