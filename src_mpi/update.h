...
//  if ((sidechain_step!=0)||(USE_ROTAMERS))
  ...
  else
  {
    ...
    {
      if (native_residue[i+1].phi!=cur_phi[i])
        native_residue[i+1].phi=cur_phi[i]*deg2rad;
      if (native_residue[i+1].psi!=cur_psi[i])
        native_residue[i+1].psi=cur_psi[i]*deg2rad;
    }
    ...
    /*  for(i=0; i<mc.loop_size; i++) {
      ...
      native_residue[mc.selected[i]].psi += mc.delta_psi_angle[i];
        //printf("up_phi:%8.3f, psi:%8.3f\n",mc.delta_phi_angle[i], mc.delta_psi_angle[i]);
        //native_residue[mc.sel_res_num+i].phi += mc.delta_phi_angle[i];
        //native_residue[mc.sel_res_num+i].psi += mc.delta_psi_angle[i];
      }*/
