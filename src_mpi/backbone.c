...
int main(int argc, char *argv[]) {
  ...
  /* OUTPUT FILE PREFIX */
const char std_prefix[] = "/PATHNAME/REMC/sim/results/1ail";   // output file prefix
  ...
  /* MPI initialization */
  MPI_Init(&argc,&argv);           // mpi begins with MPI_init and ends with MPI_Finalize()
  mpi_world_comm = MPI_COMM_WORLD;  
  MPI_Comm_size(mpi_world_comm, &nprocs);  // return num of cores : nprocs 
  MPI_Comm_rank(mpi_world_comm, &myrank);  // return index of cores:myrank

  MC_TEMP = MC_TEMP_MIN * expf(myrank*logf(10.0)/(nprocs-1));// Temperature for this processor       // from 0.1 to ... step=0.1
  sprintf(std_file,"%s_%5.3f.log",std_prefix,MC_TEMP);  

  STATUS=fopen(std_file,"w");   // STATUS records all the ouput log

  SetProgramOptions(argc,argv);  // init.h

  seed = time(NULL); 
  seed += (int) (MC_TEMP*1000) + getpid();   
  srand48(seed);  // get 48 bit calculation random number
  //fprintf(STATUS,"nprocs = %d, myrank = %d\n",nprocs,myrank);
  fprintf(STATUS,"---GENERAL---\n");
  fprintf(STATUS,"  seed:\t\t%ld\n\n", seed);
  fflush(STATUS);  // clear the std out and write all the catch into the file immediately
  ...
  InitializeMatrix(); // lattice_util.h
  InitializeProtein(); // init.h
  InitializeFraglib(); // new added
  InitializeFragRotationData(); // new added
