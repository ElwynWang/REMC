...
struct contact_data {
  unsigned char contacts:1;    // allocate bits of this type
...
Float FRAG_MOVE; // new added
Float NOISE_RANGE_PHI;
Float NOISE_RANGE_PSI;
int USE_CRANK_MOVE;
Float fragmove_accepted=1.0;
Float fragmove_rejected=0.0;
Float fragmove_weight;
...
char native_file[100], structure_file[100], triplet_file[100], sctorsion_file[100], sec_str_file[100], template_file[100], pdb_out_file[100], amino_data_file[100], rotamer_data_file[100], potential_file[100], atom_type_file[150], helicity_data[200],hydrogen_bonding_data[200], fraglib_file[200];
...
int naccepted_frag,naccepted_yang,naccepted_loop,naccepted_local,n_frag,n_yang,n_loop,n_local;
int sidemovedone;
struct monte_carlo {
  int backbone_move;
  int sel_res_num;
  ...
  int torsion;
  Float delta_angle[4];
  Float delta_phi_angle[20]; // change
  Float delta_psi_angle[20];  // change
  int loop_size;
  int sel_triplet;
  int selected[20]; //change
} mc;
...
//new added
short **frag_left_rotate_natoms;
short ***frag_left_rotate_atom;
char ***frag_left_not_rotated;
short **frag_right_rotate_natoms;
short ***frag_right_rotate_atom;
char ***frag_right_not_rotated;
...
float *Enode, *Tnode;
float delta_E, delta_T, delta_all;
