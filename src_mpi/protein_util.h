...
void CenterProtein(struct atom **protein, int num_atoms) {   // to locate the protein in the center (0,0,0)
...
void align_drms(struct atom *protein1, struct residue *residue1, struct atom *protein2, struct residue *residue2, int *map1, int *map2, int align_len, Float *rms) {
  // calclulate drms for two aligned gragments
...
void SSdRms(struct atom *protein1, struct atom *protein2, int num_atoms, Float *sc_rms, Float *bb_rms, int first_res, int last_res) { // only calculate fragment with the range of first_res and last_res
  Float scrms_tot, bbrms_tot, d1, d2;
  int sc_count=0, ca_count=0, i, j;
...
void AddCB(struct atom *protein, struct residue GLY, struct atom *CB) {  // for GLY
