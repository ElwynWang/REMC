...
//  mc.is_phi = 1;
//fprintf(STATUS,"%f\n", deg2rad*2.);
  step_size = YANG_SCALE*2.*deg2rad*GaussianNum(); // Yang_Scale default is 30 in cfg
...
//  fprintf(STATUS,"Local move at %d\n", n0);
  ...
  get_orgco(rorg_n, rorg_a, rorg_c, rorg_o, rorg_s, ns, n0);  // n:N a:CA c:C o:O s:Side chain   record 5 residues in the original native strucutre info
  get_coord(r_n, r_a, r_c, r_o, r_s, ns, n0);                // record 5 residues in the native structure structure info
  ...
  yangloop(r_n, r_a, r_c, r_o, r_s, ns, b_len, b_ang, n0, step_size, res_name, n_soln);  // do the rotate in the Cartesian coordinates system by Tripeptide Loop Closure Algorithm
  ...
  if(*n_soln > 0)                                  // check errors after the rotations
   {
...
//    fprintf(STATUS,"passed\n");
  ...
  if(*n_soln > 0)        // if there's no error listed above, then record all the rotates and info.
   {
    put_coord(r_n, r_a, r_c, r_o, r_s, ns, n0);  // use the new coordinates of the 4-residue to change the old ones in the native structure 
    yang_rotate(n0, mc.is_phi);    // record the rotated atoms and residues
    all_rotated_natoms = yang_rotated_natoms; 
    all_rotated_atoms = yang_rotated_atoms;
    UpdateLattice(yang_rotated_natoms, yang_rotated_atoms);  // update latice model
    NewDeltaContacts(yang_rotated_natoms, yang_rotated_atoms, yang_not_rotated);  // calculate contacts
    success++;
   }
...
// driver angle
  if(mc.is_phi)
   {
    driver_rot(r_c[3], r1_c[3], r_a[4], r_n[4], dih_ch);  // dih_ch: the range of angle change
