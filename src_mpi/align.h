...
void SetAlignContactDistance() {            // a,b is a range of contact allowance
  Float rad1, rad2;
  Float temp;
  int i, j, k;
  ...
  if (!USE_GO_POTENTIAL) {
    ...
    if (MAX_TYPES==84) {
      ...
      for (j = 79; j <= 83; ++j)
	for (i = 0; i < MAX_TYPES; ++i)   // for the last few items but why???
	  if ((i < 79) || (i > 83))
...
void SetupAlignmentStructure() {   // Very Very Important!!! Initilize some important matrix!!! struct_native, struct_residue, struct_data
  int i;

  struct_native = (struct atom *) calloc(MAX_ATOMS,sizeof(struct atom));  // record all atoms in a protein (pdb file)
  ReadNative(structure_file,struct_native,&struct_natoms); // init.h
  ...
  for(i=0; i<struct_natoms; i++) 
    if (!strncmp(struct_native[i].atomname,"CA",2)) 
      struct_nresidues++;
  struct_residue = (struct residue *) calloc(struct_nresidues,sizeof(struct residue)); // record residues info in a protein
  GetResidueInfo(struct_native, struct_residue, struct_nresidues, struct_natoms);   // init.h
  struct_data = (struct contact_data **) calloc(struct_natoms,sizeof(struct contact_data *));  // natom*natom matrix of contact data
  for(i=0; i<struct_natoms; i++)
...
void SetupAlignmentPotential() {
  ...
  for (i = 0; i < nalign; ++i)
	...
	if ((map_to_struct[i] >= str_segment[m].a) && (map_to_struct[i] <= str_segment[m].b) && (map_to_struct[j] >= str_segment[m].a) && (map_to_struct[j] <= str_segment[m].b))
	  // the above same segments criteron is confused!
    same_segment = 1;
